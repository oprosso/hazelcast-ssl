    ...
    public void init(final Properties properties, final boolean forClient) throws Exception {
        ...
        if (configuredCipherSuites != null) {
            ...
            if (this.cipherSuites.length < 1) {
                throw new HazelcastException("No configured SSL cipher suite name is valid. Check if configured values " + Arrays.toString(configuredCipherSuites) + " contain supported values: " + Arrays.toString(supportedCipherSuites));
            }
        }
